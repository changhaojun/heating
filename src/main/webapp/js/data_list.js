	...
	function labelCompany() {
		$.ajax({
			...
			success: function(data) {
				...
				maxSize = data.length;
				for (var i in data) {
					var tagName = '<th data_tag="' + data[i]._id + '">' + data[i].tag_name + '</th>';
					$("#dtuList thead>tr .handle").before(tagName);
					tagId.push(data[i]._id);
				}
				dataFixed(companyId)
			}
		});
	...
	function dataFixed(companyId) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				var m = -1;
				for (var i in data) {
					var tableDataStr = '<tr><td class="name" onclick="branch(' + i + ')"><b>' + (parseInt(i) + 1) + '</b>' + data[i].name + '</td><td>' + data[i].station_count + '</td><td>' + data[i].heating_area + '</td>' + '<td class="handdle"><i class="fa fa-sign-in" data-toggle="tooltip" data-placement="top" title="进入" onclick="branch(' + i + ')"></i></td></tr>';
					...
					for (var m in data[i].value) {
						if (data[i].value[m].data_value != null) {
							for (var t = 0; t < $('th').length; t++) {
								if ($('th').eq(t).attr("data_tag") == data[i].value[m].data_tag) {
									q = t;
									$("#dtuList tbody tr").eq(i).find("td").eq(q).html(data[i].value[m].data_value).append('<i class="fa fa-laptop hidden" data-toggle="tooltip" data-placement="top" title="查看" ></i>');
					...
					companyIdArr.push(data[i].id);
					idName = companyIdArr;
					hotName.push(data[i].name);
				}
	...
	function icon() {
		...
		$("#dtuList>tbody tr").each(function(i, ele) {
			...
			/*if(tagLevel==3){
				...
				$(this).find("td i.fa-laptop").each(function(j,ele){
					$(this).closest("td").on({
						"mouseenter":function(){
	...
	function dataBranch(n) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				//console.log(maxSize);
				for (var i in data) {
					var tableDataStr = '<tr><td class="name" onclick="station(' + i + ')"><b>' + (parseInt(i) + 1) + '</b>' + data[i].name + '</td><td>' + data[i].station_count + '</td><td>' + data[i].heating_area + '</td>' + '<td class="handdle"><i class="fa fa-sign-in" data-toggle="tooltip" data-placement="top" title="进入" onclick="station(' + i + ')"></i></td></tr>';
					$("#dtuList tbody").append(tableDataStr);
					var num = $('th').length - 4;
					for (var j = 0; j < num; j++) {
						var tdStr = '<td class="color">-<i class="fa fa-laptop hidden" data-toggle="tooltip" data-placement="top" title="查看" ></i></td>';
					...
					recordCode = data[1].company_code;
					branchCode.push(data[i].company_code)
					branchId.push(data[i].id);
					idName = branchId;
					hotName.push(data[i].name);
	...
	function labelStation(n) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$(".column ul").empty();
				for (var i in data) {
					$(".column ul").append('<li><label><input type="checkbox" data_tag="' + data[i]._id + '" status="0">' + data[i].tag_name + '</label></li>');
				...
				var num = 5;
				for (var j = 0; j < num; j++) {
					var tagName = '<th class="' + data[j]._id + '" data_tag="' + data[j]._id + '">' + data[j].tag_name + '</th>';
					$("#dtuList thead>tr .handle").before(tagName);
					tagId.push(data[j]._id);
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (initStatus == true) {
					...
					//console.log(112)
					tagId.push($(".column ul input").eq(clickNum).attr("data_tag"))
					$("#dtuList thead>tr .handle").before('<th data_tag="' + $(".column ul input").eq(clickNum).attr("data_tag") + '">' + dynamicData[clickNum] + '</th>');
					if (data != '') {
						for (var x in data) {
							for (var i = 0; i < $("tbody>tr").length; i++) {
								if ($("tbody>tr").eq(i).attr('id') == data[x].station_id) {
									for (var m in data[i].value) {
										//console.log(m)
										if (data[x].value[m].data_value != null) {
											for (var t = 0; t < $('th').length; t++) {
												if ($('th').eq(t).attr("data_tag") == data[x].value[m].data_tag) {
													$("#dtuList tbody tr").eq(i).find('.handdle').before('<td>' + data[x].value[m].data_value + '<i class="fa fa-laptop hidden" data-toggle="tooltip" data-placement="top" title="查看" ></i></td>')

												}
											}
										} else {
											$("#dtuList tbody tr").eq(i).find('.handdle').before('<td>-</td>')
										}
								...
								//console.log(m)
									if(data[i].value[m].data_value !=null){
										for(var t=0;t<$('th').length;t++){
											if($('th').eq(t).attr("data_tag")==data[i].value[m].data_tag){
												q=t;
												if($("tbody>tr").eq(i).attr('id')==data[i].station_id){
													$("#dtuList tbody tr").eq(i).find('.handdle').before('<td>'+data[i].value[m].data_value+'<i class="fa fa-laptop hidden" data-toggle="tooltip" data-placement="top" title="查看" ></i></td>')
												}
											}
										}
									}else{
